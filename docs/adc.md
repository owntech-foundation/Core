...
There are four signal that can be used as ADC trigger in the PWM API i.e. these trigger will start an ADC conversion : ADC_TRIG1, ADC_TRIG2, ADC_TRIG3, ADC_TRIG4.

The PWM is generated by a carrier (see [here](https://owntech-foundation.github.io/Documentation/core/docs/pwm/) for more detail). When using a comparator to compare a certain constant value with the carrier, we can gnerate an event when enabling a trigger on the PWM. For example :
...
We are enabling and linking ADCTRIG_1 to PWMA. That means that when the comparator value and the carrier value of the PWMA we can generate an event named ADCTRIG_1 that can be used to start an ADC conversion : 

![ADC trigger](images/hrtim_adc_trigger_schema.svg)

This comparator value can be changed via the function `spin.pwm.setAdcTriggerInstant(PWMA, 0.5)` for example means that we trigger an event at half the switching period and thins event will be used to start an adc conversion.

#### hrtim_eevx

An ADC conversion can be started either by software (i.e. with a function) or hardware by an external event. There are multiple external event, but there are special external event related to PWM : `hrtim_eev1`, `hrtim_eev2`, `hrtim_eev3`, `hrtim_eev4`.

Each of this external event are connected to the event generated by ADCTRIG_x, for example hrtim_eev1 are connected to ADCTRIG_1...etc.
...
spin.adc.configureTriggerSource(1, hrtim_eev1);
```
The line above means that we connected ADC**1** to the external event **hrtim_eev1**, which is connected to the event generated by **ADCTRIG1** when the comparator (set by the trigger value) is equal to the carrier of **PWMA**.
